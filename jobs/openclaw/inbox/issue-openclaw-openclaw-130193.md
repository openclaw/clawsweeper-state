---
repo: openclaw/openclaw
cluster_id: issue-openclaw-openclaw-130193
mode: autonomous
job_intent: implement_issue
allowed_actions:
  - comment
  - label
  - fix
  - raise_pr
blocked_actions:
  - close
  - merge
require_human_for:
  - close
  - merge
canonical:
  - #130193
candidates:
  - #130193
cluster_refs:
  - #130193
allow_instant_close: false
allow_fix_pr: true
allow_merge: false
allow_unmerged_fix_close: false
allow_post_merge_close: false
require_fix_before_close: false
security_policy: central_security_only
security_sensitive: false
target_branch: clawsweeper/issue-openclaw-openclaw-130193
source: issue_implementation
trigger_source: review_reproducible_bug
clawsweeper_report_path: records/openclaw-openclaw/items/130193.md
source_issue_repo: "openclaw/openclaw"
source_issue_number: 130193
source_issue_revision_sha256: "9cd0e16b1c0385d48f0f290fd3172f8dc6532d6b03902969a871edf62ab1b21a"
required_pr_labels:
  - clawsweeper:autogenerated
  - clawsweeper:autofix
---

# ClawSweeper issue implementation candidate

ClawSweeper Repair should create or update one implementation PR from `clawsweeper/issue-openclaw-openclaw-130193`.

Source issue: https://github.com/openclaw/openclaw/issues/130193
Title: Sandboxed read tool rejects paths in the agent's own sandbox.docker.binds when workspaceOnly is true (guard mount table ignores binds)

## Maintainer Context

- Trigger source: review_reproducible_bug
- ClawSweeper review report: records/openclaw-openclaw/items/130193.md
- Review report URL: https://github.com/openclaw/clawsweeper-state/blob/state/records/openclaw-openclaw/items/130193.md
- Maintainer instruction: This was selected by ClawSweeper's high-confidence bug-fix lane.

Review report: https://github.com/openclaw/clawsweeper-state/blob/state/records/openclaw-openclaw/items/130193.md
Category: bug
Reproduction: source_reproducible (high)
Feature/config/product blockers: false.

Bug-fix boundary:

- fix broken existing behavior only
- do not add config options, feature modes, providers, broad UX changes, or product policy
- reproduce first; if reproduction fails on latest main, stop and report that blocker

Review work prompt:

Repair the workspace-only sandbox read path so a declared read-only Docker bind is permitted by the same canonical mount model used by the sandbox filesystem bridge. Reproduce the reported failure before editing. In src/agents/core-coding-tools.ts, replace the partial read-only guard mount derivation with canonical read-only non-workspace mounts from src/agents/sandbox/fs-paths.ts; do not add configuration, broaden write/edit/apply_patch access, or permit undeclared paths. Add an assembled-tool regression using workspaceAccess:none and a real declared :ro bind, proving read succeeds, writes remain unavailable/rejected, and an undeclared absolute path still fails. Do not edit CHANGELOG.md; record user-visible release context in the PR body.

Likely files:

- src/agents/core-coding-tools.ts
- src/agents/agent-tools.create-openclaw-coding-tools.test.ts
- src/agents/sandbox/fs-paths.ts

Validation:

- node scripts/run-vitest.mjs src/agents/agent-tools.create-openclaw-coding-tools.test.ts
- node scripts/run-vitest.mjs src/agents/sandbox/fs-paths.test.ts
- node scripts/run-vitest.mjs src/agents/agent-tools.read.workspace-root-guard.test.ts
- node scripts/check-changed.mjs -- src/agents/core-coding-tools.ts src/agents/agent-tools.create-openclaw-coding-tools.test.ts

## Operator Prompt

Use the source issue as the product request or bug report. Verify the request is
still valid on latest `openclaw/openclaw@main`, inspect nearby code, and make the
narrowest implementation that directly satisfies the issue. If the issue is too
broad, underspecified, security-sensitive, already fixed, or not safely
implementable by automation, do not change code; report the exact blocker.

This job came from ClawSweeper's reproducible bug lane. Treat it as bug-only:

- Fix only broken existing behavior that is already expected by current docs,
  tests, CLI/API contracts, or clearly established behavior.
- Stop without code changes if the issue requires a new feature, new config
  option, new mode, new dependency, broad UX/product judgment, or a policy
  decision.
- Start by reproducing the bug or adding a failing regression test. If the bug
  cannot be reproduced on latest main, report the blocker instead of opening a
  PR.
- The final PR must stay narrow and should carry the
  `clawsweeper:autogenerated` and `clawsweeper:autofix` labels; the executor also
  applies these labels after opening or updating the PR.




When code changes are appropriate, emit a fix artifact with
`repair_strategy: "new_fix_pr"`, `source_prs: []`, this issue in
`linked_refs`, and validation commands for the touched surface. Keep working
until the PR branch is locally validated and ready for clean required CI; if CI
or type/test validation fails, fix and rerun until it passes or a concrete
external blocker is proven.


## Guardrails

- Do not merge.
- Do not close the issue from this lane.
- Keep one PR for this issue; reuse `clawsweeper/issue-openclaw-openclaw-130193` if it already exists.
- Keep the diff narrow and avoid unrelated refactors.
- Preserve issue context and link https://github.com/openclaw/openclaw/issues/130193 in the PR body.
- Use a closing reference for https://github.com/openclaw/openclaw/issues/130193 when the implementation satisfies the issue.
- Preserve release-note context in the PR body or commit message when the
  target repo expects it.
