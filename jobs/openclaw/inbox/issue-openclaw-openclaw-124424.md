...
Repair the Control UI New session flow so an unpersisted bootstrap route is not sent as parentSessionKey. Reuse the selected persisted session-row abstraction to preserve parent linkage and parallel-child behavior for durable selected sessions. Do not weaken Gateway validation for explicit parents, forks, or command-hook lifecycle operations. Add a regression that starts from an empty session list and asserts New session omits parent linkage, plus sibling coverage for a durable selected parent. Include sanitized real Control UI proof and release-note context in the PR body; do not edit CHANGELOG.md.
