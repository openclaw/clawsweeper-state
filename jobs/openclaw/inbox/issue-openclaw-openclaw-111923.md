...
Repair the existing memory-core concept-tag invariant: numeric-only values, decimals, and numeric ranges must not become stored concept tags or visible REM themes. Keep meaningful mixed alphanumeric technical identifiers such as `s3` and `css3`; do not add a config option, stop-word policy surface, migration, or SQLite schema change. Inspect whether existing short-term records need the same validity guard at REM rendering or can be safely rebuilt as transient state. Add focused regression coverage for numeric compounds and the REM output path, preserving the existing managed-block ingestion guard. Do not edit `CHANGELOG.md`; describe user-visible behavior in the PR body.
