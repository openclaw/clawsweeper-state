...
Repair the remaining connection-test defect. Preserve active-session model patch behavior. At the generic auth-probe selector, ensure catalog fallback does not choose a deprecated model when no exact candidate exists; retain explicit direct candidates, do not special-case Ollama, and do not alter normal routing or credential precedence. Add a regression with a configured credential, no exact candidate, and a deprecated-first catalog that selects the active row. Do not edit CHANGELOG.md.
