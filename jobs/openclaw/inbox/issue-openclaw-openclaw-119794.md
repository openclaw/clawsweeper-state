...
Repair FIFO admission for https://github.com/openclaw/openclaw/issues/119794. When an earlier active steer ends before transcript commitment and becomes a follow-up, preserve or atomically transfer its next-session admission ahead of later visible inbound work. Add a deferred-boundary regression proving A starts before B, and cover terminal cancellation/overflow releasing the reservation. Preserve successful active steering, explicit interrupt behavior, and existing retry/compaction handling; do not add queue configuration or channel-specific workarounds.
