...
target_branch: clawsweeper/issue-openclaw-openclaw-92199
source: issue_implementation
trigger_source: review_reproducible_bug
clawsweeper_report_path: records/openclaw-openclaw/items/92199.md
source_issue_repo: "openclaw/openclaw"
source_issue_number: 92199
source_issue_revision_sha256: "1e38b7b77cfd42af0a2afef89572030c4a453b0aeb259ffa7024cfa81a728583"
required_pr_labels:
...
ClawSweeper Repair should create or update one implementation PR from `clawsweeper/issue-openclaw-openclaw-92199`.

Source issue: https://github.com/openclaw/openclaw/issues/92199
...
- ClawSweeper review report: records/openclaw-openclaw/items/92199.md
- Review report URL: https://github.com/openclaw/clawsweeper-state/blob/state/records/openclaw-openclaw/items/92199.md
- Maintainer instruction: This was selected by ClawSweeper's high-confidence bug-fix lane.

Review report: https://github.com/openclaw/clawsweeper-state/blob/state/records/openclaw-openclaw/items/92199.md
Category: bug
...
Repair the ACP final-only lifecycle so visible text remains buffered for the entire dispatch-owned turn rather than flushing and resetting for every streamed `done` or `error`. Keep WeChat channel-agnostic: repair `src/auto-reply/reply/acp-projector.ts` and let `src/auto-reply/reply/dispatch-acp.ts` own terminal settlement. Add focused regression coverage for multiple terminal-like stream events before outer completion and verify one merged final payload includes all text. Do not add config, change the documented live-mode behavior, or revive the closed unmerged patches. Include release-note context in the PR body; do not edit CHANGELOG.md.
...
- Keep one PR for this issue; reuse `clawsweeper/issue-openclaw-openclaw-92199` if it already exists.
- Keep the diff narrow and avoid unrelated refactors.
- Preserve issue context and link https://github.com/openclaw/openclaw/issues/92199 in the PR body.
- Use a closing reference for https://github.com/openclaw/openclaw/issues/92199 when the implementation satisfies the issue.
