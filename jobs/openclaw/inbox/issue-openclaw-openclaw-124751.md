...
Repair the shared Apple chat path so a `session.message` envelope preserves its existing `runId` and the reconciliation code adopts the canonical assistant row over a partial live `chat.final` for that exact run. Add a regression for partial-live/full-canonical content and both arrival orders; retain the scoped content fallback for older Gateways that omit the identity and do not globally deduplicate assistant text. Do not alter Gateway persistence, providers, or CLI transcript behavior. Do not edit CHANGELOG.md; it is release-owned.
...
- Run the iOS Gateway chat transport test target with a session-message envelope containing `runId`.
- Record a redacted real iPhone-to-Gateway transcript showing one assistant row for a partial-live/full-canonical turn.
