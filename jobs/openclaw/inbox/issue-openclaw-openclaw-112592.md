...
Repair provenance only for internal completion handoffs. Preserve `INTERNAL_MESSAGE_CHANNEL` and all `channel` routing behavior. Add a named internal provenance source in `src/sessions/input-provenance.ts`, use it as the fallback in queued and direct subagent announcements, and remove explicit routing-sentinel values from internal completion producers. Keep `sessions_send` provenance sourced from its actual requester channel. Extend existing boundary tests so queued/direct completion metadata and session-history labels show `internal`; ensure the regression fails before the repair. Do not add config, a migration, runtime fallback readers, or CHANGELOG.md edits.
