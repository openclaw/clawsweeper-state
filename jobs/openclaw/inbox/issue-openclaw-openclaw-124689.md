...
Repair #124689 at the Gateway probe-planning owner. Trace `models.probe` through `runAuthProbes` and `selectProbeModel`; ensure a requested provider cannot fall from nonmatching global defaults to an arbitrary stale catalog row, while preserving deliberate provider-specific priority and predictable no-model handling. Add a regression that fails before the fix for configured API-key credentials with provider-mismatched candidates and a retired catalog row. Do not change the active-session model picker, add configuration, add a migration, or mask probe failures in the UI.
