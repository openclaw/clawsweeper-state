...
target_branch: clawsweeper/issue-openclaw-openclaw-122615
source: issue_implementation
trigger_source: review_reproducible_bug
clawsweeper_report_path: records/openclaw-openclaw/items/122615.md
source_issue_repo: "openclaw/openclaw"
source_issue_number: 122615
source_issue_revision_sha256: "f63143cdcc381eb5864fdac99baca23b87592fb1b9a8363398192eb02232740b"
required_pr_labels:
...
ClawSweeper Repair should create or update one implementation PR from `clawsweeper/issue-openclaw-openclaw-122615`.

Source issue: https://github.com/openclaw/openclaw/issues/122615
Title: Subagent announce delivery drops failure reason for dropped vs none outcomes

## Maintainer Context

- Trigger source: review_reproducible_bug
- ClawSweeper review report: records/openclaw-openclaw/items/122615.md
- Review report URL: https://github.com/openclaw/clawsweeper-state/blob/state/records/openclaw-openclaw/items/122615.md
- Maintainer instruction: This was selected by ClawSweeper's high-confidence bug-fix lane.

Review report: https://github.com/openclaw/clawsweeper-state/blob/state/records/openclaw-openclaw/items/122615.md
Category: bug
...
Repair the subagent announce path so an active requester queue refusal retains its authoritative closed reason through dispatch and durable registry state instead of becoming `sink_unavailable`. Preserve the intentional no-direct-fallback behavior for a live requester. Extend the existing delivery refusal table and lifecycle persistence coverage; establish the failing current-main regression before editing. Do not add configuration, alter delivery policy, or edit release-owned CHANGELOG.md.
...
- node scripts/run-vitest.mjs src/agents/subagents/announce/subagent-announce-dispatch.test.ts
...
- Keep one PR for this issue; reuse `clawsweeper/issue-openclaw-openclaw-122615` if it already exists.
- Keep the diff narrow and avoid unrelated refactors.
- Preserve issue context and link https://github.com/openclaw/openclaw/issues/122615 in the PR body.
- Use a closing reference for https://github.com/openclaw/openclaw/issues/122615 when the implementation satisfies the issue.
