...
target_branch: clawsweeper/issue-openclaw-openclaw-121513
source: issue_implementation
trigger_source: review_reproducible_bug
clawsweeper_report_path: records/openclaw-openclaw/items/121513.md
source_issue_repo: "openclaw/openclaw"
source_issue_number: 121513
source_issue_revision_sha256: "2145ea1e6f21f88a45b86ccb0c5d7ca1fc0031f4ce5ae2533cb2599c85441bca"
required_pr_labels:
...
ClawSweeper Repair should create or update one implementation PR from `clawsweeper/issue-openclaw-openclaw-121513`.

Source issue: https://github.com/openclaw/openclaw/issues/121513
Title: [Bug]: agents.list[].identity (name/emoji) never applied to outbound Slack messages despite chat:write.customize scope and correct config

## Maintainer Context

- Trigger source: review_reproducible_bug
- ClawSweeper review report: records/openclaw-openclaw/items/121513.md
- Review report URL: https://github.com/openclaw/clawsweeper-state/blob/state/records/openclaw-openclaw/items/121513.md
- Maintainer instruction: This was selected by ClawSweeper's high-confidence bug-fix lane.

Review report: https://github.com/openclaw/clawsweeper-state/blob/state/records/openclaw-openclaw/items/121513.md
Category: bug
...
Repair the established identity-delivery contract: final replies from the generic agent RPC delivery path must carry the active agent’s configured outbound identity into channel adapters. First add a regression that fails on current main for explicit Slack delivery from a non-default agent and proves the durable outbound call receives that agent’s name and emoji. Resolve identity at src/agents/command/delivery.ts using deliveryAgentId, preserve explicit caller identity and channel relay fallback precedence, and add no config. Do not edit CHANGELOG.md; put release-note context in the PR body.
...
- Keep one PR for this issue; reuse `clawsweeper/issue-openclaw-openclaw-121513` if it already exists.
- Keep the diff narrow and avoid unrelated refactors.
- Preserve issue context and link https://github.com/openclaw/openclaw/issues/121513 in the PR body.
- Use a closing reference for https://github.com/openclaw/openclaw/issues/121513 when the implementation satisfies the issue.
