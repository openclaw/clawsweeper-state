...
Repair this source-proven Cron reliability bug. Preserve the shared canonical `timeout` failover reason and do not add a Cron-only persisted `network` reason or a new config/default surface. Extend the shared transport classification for the evidenced raw stream/connection-close forms such as `stream disconnected` and `premature close`, then prove the Cron timer takes its existing bounded retry path. Add focused regression coverage for the new forms and for no false-positive retry of permanent provider errors. Keep Cron protocol conformance aligned and do not edit `CHANGELOG.md`.
